...
即使软件开发已经取得了巨大进步，Linux 内核仍然是一套非常复杂的代码。开发者和维护者不断向内核加入新特性，其他爱好者和学习者则持续尝试理解这些机制，并解决其中的疑惑。

自然地，关于Linux及其内部机制已有大量文献，从一般的系统管理到内核编程。

几十年来，已经出版了数百本书籍，涵盖了众多重要的操作系统主题，例如进程创建、线程、内存管理、虚拟化、文件系统实现以及 CPU 调度。本书聚焦 Linux 存储技术栈及其多层结构。

我们将从 VFS 开始介绍。VFS 在进程访问文件系统数据时起着关键作用。由于本书会自上而下介绍整个存储栈，因此深入理解虚拟文件系统（Virtual Filesystem）至关重要，因为它是内核中 I/O 请求的起点。我们将介绍用户空间和内核空间的概念，理解系统调用，并了解 Linux 中“一切皆文件”的理念如何与虚拟文件系统相关联。
...
系统调用和应用程序接口（API）之间经常会被混淆。API 是提供给程序使用的一组编程接口，定义了两个组件之间通信的方法。API 在用户空间中实现，并说明如何获得特定服务。系统调用则是更底层的机制，它通过中断向内核发出明确请求。在 Linux 中，系统调用接口由标准 C 库提供。
...
| `execve()` | 执行 pathname 指向的程序 |
| `access()` | 检查调用进程是否可以访问指定文件路径 |
...
总之，在 Linux 中，系统调用接口实现了一组通用方法，用户空间应用程序可以通过这些方法访问内核空间中的资源。

## 文件系统的必要性

一个标准文件系统是一组数据结构，它们决定了用户数据如何在磁盘上组织。最终用户可以通过常规文件访问方法与文件系统交互，并执行常见任务。每个 Linux 系统都至少提供一种文件系统。主流 Linux 发行版通常默认使用 XFS 或 Ext4，这些文件系统功能丰富，并且在日常使用中被认为稳定可靠。

然而，Linux 并不只支持这两种文件系统。Linux 的优势之一是支持多种文件系统，它们在不同场景下都可以作为 Ext4 或 XFS 的替代方案。因此，Linux 也能够与其他操作系统共存。其他常见文件系统包括 Ext4 的早期版本 Ext2、Ext3，以及 Btrfs、ReiserFS、OpenZFS、FAT 和 NTFS。
...
为了确保应用程序在面对不同文件系统时不会遇到障碍，Linux 内核在终端用户应用程序和用于存储数据的文件系统之间实现了一个层次结构。这一层被称为 VFS。VFS 不是像 Ext4 或 XFS 那样的标准文件系统，因此有些人更喜欢使用“虚拟文件系统切换”（Virtual Filesystem Switch）这个术语。
