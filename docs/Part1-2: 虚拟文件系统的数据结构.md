...
本地 Linux 文件系统认为目录也是一种文件，并且像文件一样存储它们。像其他常规文件一样，目录也会被分配一个 inode。文件和目录的 inode 有一个区别：对于目录，inode 中的类型字段会标识它是目录。请记住，前面对 inode 的讨论已经说明，inode 包含大量文件元数据，但不包含文件名。文件名存放在目录中。可以将目录视为一种包含表格的特殊文件，该表格记录文件名及其对应的 inode 号。
